...
const destSaveLoc = path.join(__dirname, destLocPath); // E:\work\downtolocal\example\www\static\localres\
...
function matchDemo(curPath, body) {
  //读取文件内容，进行内容的替换
  //console.log('locMap.searchPath',locMap.searchPath)// /example
  //console.log('reg',locMap.searchPath.replace(/\//g, "\\\\") + "\\\\(.*)") // \\example\\(.*)
  //console.log('curPath',curPath)// E:\work\downtolocal\example\demo.html
  //想要的结果：E:\work\downtolocal\example\demo.html （\\example\\(.*)） -> dirname
  // path.sep \ -> E:/work/downtolocal/example/demo.html
  // var dirname = curPath.match(
  //   new RegExp(locMap.searchPath.replace(/\//g, "\\\\") + "\\\\(.*)")
  // ); //没有目录归属的情况
  //console.log('mod',locMap.searchPath.replace(/\//g, path.sep) + path.sep +"(.*)") // \example\(.*)
  var dirname = curPath.split(path.sep).join('/').match(
    new RegExp(locMap.searchPath + "\/(.*)")
  );

  var proDestDir;
  console.log('dirname',dirname[1])
